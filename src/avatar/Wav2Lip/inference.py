...
def generate(face_path, audio_path, outfile_path):
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen,
                                                                    total=int(
                                                                        np.ceil(float(len(mel_chunks)) / wav2lip_batch_size)))):
        if i == 0:
    ...
    command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(audio_path, avi_path, outfile_path)
    subprocess.call(command, shell=platform.system() != 'Windows')
    

dir_path = os.path.dirname(os.path.abspath(__file__))
model_path = os.path.join(dir_path, "checkpoints", "wav2lip.pth")
avi_path = os.path.join(dir_path, "temp", "result.avi")
